...
import { ArrowLeft, ArrowUp, ArrowsClockwise, Binoculars, Briefcase, CaretRight, ChatCircleDots, Check, ClockCounterClockwise, GearSix, MagnifyingGlass, Plus, Sparkle, Stack, StarFour, StopCircle, TerminalWindow, WarningCircle, X } from '@phosphor-icons/react';
...
export default function Prototype() {
  ...
  return <div className="hermes-app">
    <header className="app-header">
      {selected?<><button className="icon-button" aria-label="Back to agents" onClick={()=>navigate('agents')}><ArrowLeft weight="bold"/></button><button className="profile-heading" onClick={()=>setSheet('profile')}><Avatar id={selected} small/><span><strong>{active?.name||selected}</strong><small>{busyRun?statusText(busyRun.status):active?.connected?'Connected to Omarchy':'Gateway offline'}</small></span></button><button className="icon-button" aria-label="Conversation history" onClick={()=>void showHistory()}><ClockCounterClockwise weight="duotone"/></button><button className="icon-button" aria-label="New conversation" onClick={fresh}><Plus weight="bold"/></button></>:<><img className="brand" src="/hermes-emblem.png" alt=""/><div className="heading"><h1>{tab==='agents'?'Hermes Relay':tab==='work'?'Work':'Connection'}</h1><p>{loading?'Finding your agents…':tab==='work'?'Tasks started in this app':`${online} connected · Omarchy`}</p></div><button className="icon-button" aria-label="Refresh connection" onClick={()=>{setError('');void refresh();}}><ArrowsClockwise weight="bold"/></button></>}
    </header>
    {error&&<div className="error-banner" role="alert"><span>{error}</span><button className="icon-button" aria-label="Dismiss error" onClick={()=>setError('')}><X weight="bold"/></button></div>}
    <div className={'scroll-region '+(selected?'chat-region':'')}><MobileScroll className="hermes-scroll"><main className="page-content" style={{paddingBottom:selected?112:106}}>
      {!selected&&tab==='agents'&&<>
        <div className="search"><MagnifyingGlass weight="bold"/><KeyboardInput aria-label="Search agents" placeholder="Search your agents" value={filter} onChange={e=>setFilter(e.target.value)}/></div>
        {pending.length>0&&<button className="attention-row" onClick={()=>setDetail(pending[0])}><WarningCircle weight="duotone"/><span>{pending.length} request{pending.length===1?'':'s'} need{pending.length===1?'s':''} you</span><CaretRight weight="bold"/></button>}
      ...
      {selected&&<>
        {Boolean(pendingSend)&&<button className="attention-row" disabled={sending} onClick={()=>void recoverSend()}><WarningCircle weight="duotone"/><span>Recover unconfirmed send</span><CaretRight weight="bold"/></button>}{busyRun&&<button className="activity-row" onClick={()=>setDetail(busyRun)}><span className={'status-dot '+(busyRun.connectionError?'amber':'')}/><span>{busyRun.connectionError?'Connection needs checking':statusText(busyRun.status)+' on Omarchy'}</span><CaretRight weight="bold"/></button>}
        ...
        {busyRun?.status==='waiting_for_approval'&&<button className="attention-row" onClick={()=>setDetail(busyRun)}><WarningCircle weight="duotone"/><span>Review this action</span><CaretRight weight="bold"/></button>}
    ...
    <BottomSheet open={sheet!==null} onOpenChange={open=>{if(!open)setSheet(null);}} title={sheet==='history'?'Conversations':active?.name||'Agent'} description="Connected to your local Hermes profile.">
      <div className="sheet-body">{sheet==='profile'?<><Avatar id={selected||'default'}/><h3>{active?.name}</h3><p>Profile: {active?.id}</p><p>{active?.connected?'Connected to the local Hermes gateway.':active?.reason}</p><p className="muted">Uses the profile’s configured provider, tools and model. This preview does not change its configuration.</p><button className="wide-button" onClick={()=>void showHistory()}>Open conversation history</button></>:<><button className="wide-button" onClick={fresh}><Plus weight="bold"/> New conversation</button>{historyBusy&&<p>Loading history…</p>}{historyError&&<p role="alert">{historyError}</p>}{history.map(s=><button className="history-row" key={s.id} onClick={()=>{if(selected)openAgent(selected,s.id);setSheet(null);}}><ChatCircleDots weight="duotone"/><span>{s.title}</span><CaretRight weight="bold"/></button>)}{!historyBusy&&!historyError&&!history.length&&<p>No previous conversations for this profile.</p>}<p className="muted">Showing up to 30 recent conversations.</p></>}</div>
    </BottomSheet>
